...
<ul role="list" class="flex flex-wrap items-center gap-3">
    @foreach ($links as $item)
        <li>
            <a href="{{ $item->url }}" aria-label="{{ $item->label }}" class="grid size-10 place-items-center rounded-full border border-line text-muted transition-colors duration-200 hover:border-ink hover:text-ink">
                @if ($item->icon == 'email')
